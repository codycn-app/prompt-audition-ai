...
const CommentSection: React.FC<{ comment: Comment }> = ({ comment }) => {
    ...
    const author = comment.profiles ? { username: comment.profiles.username, avatarUrl: comment.profiles.avatarUrl } : getUserById(comment.userId);
...
const ImageDetailModal: React.FC<ImageDetailModalProps> = ({ 
  ...
  const author = image.profiles ? { username: image.profiles.username, avatarUrl: image.profiles.avatarUrl } : getUserById(image.userId);
      ...
      >
        ...
        >
          ...
          <div className="relative flex-shrink-0 w-full bg-black md:w-3/5">
            <img src={image.imageUrl} alt={image.prompt} className="object-contain w-full h-full" />
          </div>
